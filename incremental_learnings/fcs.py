...
from incremental_learnings.base import BaseLearner
...
class FCS(BaseLearner):
    def __init__(self, args):
        ...
        self.logs_name = "{}/{}/{}/{}/{}".format(args["model_name"], args["dataset"], init_cls, args['increment'],
                                                 args['log_name'])
    ...
    def after_task(self):
        ...
        self.save_checkpoint(os.path.join(self.logs_name,
                                          "{}_{}_{}".format(self.args["model_name"], self.args["init_cls"],
                                                            self.args["increment"])))
    ...
    def _train(self, train_loader, test_loader):
        ...
        if not resume:
            ...
            if self._cur_task == 0 and self.args["dataset"] == "imagenetsubset":
                ...
                optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, self._network.parameters()), lr=base_lr,
            ...
            if epoch % 5 != 0:
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_fkd {:.3f}, Loss_proto {:.3f}, Loss_transfer {:.3f}, Loss_contrast {:.3f}, Train_accy {:.2f}'.format(
                    self._cur_task, epoch + 1, self._epoch_num, losses / len(train_loader),
                                    ...
                                    losses_proto / len(train_loader), losses_transfer / len(train_loader),
                                    losses_contrast / len(train_loader), train_acc)
            else:
                ...
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_fkd {:.3f}, Loss_proto {:.3f}, Loss_transfer {:.3f}, Loss_contrast {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}'.format(
                    self._cur_task, epoch + 1, self._epoch_num, losses / len(train_loader),
                                    losses_clf / len(train_loader), losses_fkd / len(train_loader),
                                    losses_proto / len(train_loader), losses_transfer / len(train_loader),
                                    losses_contrast / len(train_loader), train_acc, test_acc)
